...
- Inspector lists (Variables, Columns) use `InspectorRow`: the name in the monospaced font with its type as a trailing caption, one secondary detail line, and row actions that fade in before the type caption on hover, in space that is always reserved so hovering never hides or shifts anything; each ends in a `FilterBar`. Columns shows for data files and DataFrame tabs, Variables for everything else. Column details are whole-result statistics, never page-only numbers: `LocalDataEngine.summary` (aggregate queries in bounded column batches, which also give the data tab its "of N" total) for data files, the kernel's `dfsummary` (same filter as `df`) for DataFrames. Variable details open in a popover from `info.circle`, offered only when the kernel marks the value `inspectable` (a container with nested containers or more than 10 items).
- Say each thing once. A document's header bar starts with its summary and controls, never the name its tab already shows (a database table name is fine, the tab shows the file); a caption appears on the selected item or its preview, not both.
...
- `FindState` (per document), `CellSelection` and `VariableStore` (on `AppState`) are separate observables for the same reason; the Variables panel observes only `VariableStore`, never `AppState`.
...
- Never put `.onTapGesture(count: 2)` on rows of a selectable `List`: SwiftUI then holds every single click for the double-click interval, so selection lags. Double-click and right-click go on the list with `.contextMenu(forSelectionType:menu:primaryAction:)`.
- `DataFrameRowView` draws its cells itself, so `DataFrameTableView.tile()` marks row views for display; during a live column drag AppKit only posts `columnDidResize` on mouse-up and never redraws row views on its own.
